...
func set_lava_level(lvl: float) -> void:
	...
	if lava_lvl <= 100:
		skeleton.set_bone_global_pose_override(skeleton.find_bone("lava_level"), Transform3D(Basis(), Vector3(0, 0, lava_lvl)))
		skeleton.set_bone_global_pose_override(skeleton.find_bone("lava_level_extension"), Transform3D(Basis(), Vector3(0, 0, 0)))
		skeleton.set_bone_global_pose_override(skeleton.find_bone("lava_level_extension_02"), Transform3D(Basis(), Vector3(0, 0, 0)))
	elif lava_lvl > 100 and lava_lvl < 200:
		var diff = lava_lvl - 100
		skeleton.set_bone_global_pose_override(skeleton.find_bone("lava_level"), Transform3D(Basis(), Vector3(0, 0, 100)))
		skeleton.set_bone_global_pose_override(skeleton.find_bone("lava_level_extension"), Transform3D(Basis(), Vector3(0, diff, 0)))
		skeleton.set_bone_global_pose_override(skeleton.find_bone("lava_level_extension_02"), Transform3D(Basis(), Vector3(0, 0, 0)))
	elif lava_lvl >= 200 and lava_lvl <= 300:
		var diff = lava_lvl - 200
		skeleton.set_bone_global_pose_override(skeleton.find_bone("lava_level"), Transform3D(Basis(), Vector3(0, 0, 100)))
		skeleton.set_bone_global_pose_override(skeleton.find_bone("lava_level_extension"), Transform3D(Basis(), Vector3(0, 100, 0)))
		skeleton.set_bone_global_pose_override(skeleton.find_bone("lava_level_extension_02"), Transform3D(Basis(), Vector3(0, diff, 0)))
